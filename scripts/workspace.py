...
Subcommands
-----------
  install                     Run the interactive install wizard
...
List-field semantics in install.config.yaml
-------------------------------------------
  absent / "all"   No constraint — install everything (or defer to scope).
  []               Explicit empty list — install nothing for this category.
  [a, b, …]        Install exactly those items (union with scope-derived set).

Examples
--------
    python scripts/workspace.py install                    # interactive wizard
    python scripts/workspace.py install --config install.config.yaml
    python scripts/workspace.py install --scopes stats university --language r
    python scripts/workspace.py install --dry-run

    python scripts/workspace.py status
    python scripts/workspace.py list
    python scripts/workspace.py list agents
    python scripts/workspace.py add agent classical-stats-analyst
    python scripts/workspace.py remove mcp filesystem
    python scripts/workspace.py set language r
    python scripts/workspace.py set target opencode
    python scripts/workspace.py apply
    python scripts/workspace.py apply -y
    python scripts/workspace.py apply --dry-run
"""
...
def _run_transpile(workspace: Path, config: Dict, dry_run: bool = False) -> bool:
    ...
    # Pass explicit item lists so scope_filter can respect empty-list constraints.
...
def _save_config(config: Dict, path: Path, dry_run: bool) -> None:
    ...
    List-field values of None are omitted so the saved file stays clean
    (absent key  ≡  no constraint, same as None in memory).
    """
    if dry_run:
...
def _load_config(path: Path) -> Optional[Dict]:
    """Load install.config.yaml.  Returns None if the file is missing.
...
def _default_config() -> Dict:
    """Return a config skeleton with no constraints and sensible defaults.
    ...
    def _build_filter(scope_key: str, cfg_key: str) -> Optional[Set[str]]:
        ...
        if items is not None:
            if items:
                ...
                # Explicit empty list: override to nothing
                filt = set()
...
def cmd_status(workspace: Path, project_root: Path, config_path: Path) -> None:
    ...
    if config is None:
        console.print(
            ...
            "   Run [bold]python scripts/workspace.py install[/bold] to create one.[/]"
        )
        return
...
def cmd_apply(
    ...
    if config is None:
        console.print(
            ...
            "   Run the install wizard first: python scripts/workspace.py install[/]"
        )
        sys.exit(1)
...
def main() -> None:
    # ------------------------------------------------------------------
    # Backward-compat: bare invocation with no subcommand still runs
    # the install wizard; flags like --config / --scopes are forwarded.
    # ------------------------------------------------------------------
    raw = sys.argv[1:]
