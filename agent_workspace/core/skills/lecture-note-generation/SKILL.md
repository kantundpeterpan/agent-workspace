...
```markdown
> **Definition (Unbiasedness).** An estimator $\hat{\theta}$ is *unbiased* for
> $\theta$ if $E[\hat{\theta}] = \theta$ for all values of $\theta$ in the
> parameter space.
```
...
```markdown
**Example 1.** A sample of $n = 36$ has $\bar{x} = 42.5$ and $\sigma = 12$.
Construct a 95% confidence interval for $\mu$.

**Solution.**

1. Standard error: $SE = \sigma / \sqrt{n} = 12 / 6 = 2$
2. Margin of error: $1.96 \times 2 = 3.92$
3. Confidence interval:

$$\bar{x} \pm 1.96 \cdot SE = 42.5 \pm 3.92 = (38.58,\; 46.42)$$

**Interpretation:** We are 95% confident that $\mu$ lies between 38.58 and 46.42.
```

> **Math notation:** Use `$...$` for all inline mathematical symbols and
> expressions (e.g., `$n = 36$`, `$\bar{x}$`, `$H_0$`, `$\alpha = .05$`) and
> `$$...$$` for display equations that need visual prominence or are referenced
> in the text. This applies to all output formats (Markdown/Obsidian, Quarto, LaTeX).
