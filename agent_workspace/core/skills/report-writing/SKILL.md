...
Use **LaTeX dollar notation** for all mathematical and statistical content,
regardless of output format (LaTeX, Quarto, Markdown):

| Context | Notation | Example |
|---|---|---|
| Inline symbols/results | `$...$` | `$\beta_1 = 0.42$`, `$p < .001$`, `$R^2 = .34$` |
| Display equations | `$$...$$` | Model equations, derivations, anything referenced |
| Hypotheses | inline | `$H_0: \mu_1 = \mu_2$`, `$H_1: \mu_1 \neq \mu_2$` |
| Greek letters | always dollar | `$\alpha = .05$`, `$\sigma^2$`, `$\chi^2$` |

**Example Methods sentence:**
> A linear mixed-effects model was specified as
> $$Y_{ij} = \beta_0 + \beta_1 X_{ij} + u_{0j} + \varepsilon_{ij},$$
> where $u_{0j} \sim \mathcal{N}(0, \tau^2)$ and $\varepsilon_{ij} \sim \mathcal{N}(0, \sigma^2)$.

**In LaTeX output**, dollar notation renders natively.
**In Quarto / Markdown**, enable MathJax/KaTeX with `math: true` in the YAML header.
