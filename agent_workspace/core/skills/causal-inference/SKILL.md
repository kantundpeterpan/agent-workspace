...
3. **Potential outcomes**: $Y_i(1)$ and $Y_i(0)$ for each unit
4. **Identification assumptions**: What must hold for causal interpretation?

**Key estimand definitions (use LaTeX notation throughout):**

$$\text{ATE} = E\!\left[Y_i(1) - Y_i(0)\right]$$

$$\text{ATT} = E\!\left[Y_i(1) - Y_i(0) \mid D_i = 1\right]$$

$$\text{LATE} = E\!\left[Y_i(1) - Y_i(0) \mid \text{complier}\right]$$

> **Math notation:** Use `$...$` for inline causal quantities (e.g., `$\text{ATT}$`,
> `$Y_i(1)$`, `$E[Y \mid X]$`) and `$$...$$` for display equations.
