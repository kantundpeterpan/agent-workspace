...
Write out the statistical model before fitting, using LaTeX dollar notation:

**Linear:**
$$Y_i = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + \varepsilon_i, \qquad \varepsilon_i \sim \mathcal{N}(0,\, \sigma^2)$$

**Logistic:**
$$\log\!\left(\frac{P(Y_i=1)}{1 - P(Y_i=1)}\right) = \beta_0 + \beta_1 X_1 + \beta_2 X_2$$

**Poisson:**
$$\log(\mu_i) = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i}$$

> **Math notation:** Use `$...$` for inline expressions such as `$\beta_1 = 0.42$`,
> `$R^2 = .34$`, `$p < .001$`; use `$$...$$` for display equations.
