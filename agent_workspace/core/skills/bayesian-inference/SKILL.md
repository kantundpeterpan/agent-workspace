...
> **Math notation:** Always render mathematical quantities in LaTeX dollar notation.
> Use `$...$` for inline expressions (e.g., `$\hat{R} < 1.01$`, `$\beta_1 = 0.43$`)
> and `$$...$$` for display equations (e.g., `$$y_i \sim \mathcal{N}(\mu_i, \sigma)$$`).
